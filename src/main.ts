...
const vi: Record<string, string> = {
  ...
  caseStudy:'Xem chi tiết',mapPeople:'Con người',mapWorkflows:'Quy trình',mapClarity:'Rõ ràng',roberpLine:'Giúp công việc vận hành hằng ngày rõ ràng hơn.',glLine:'Đồng hành từ ngày mua nhà đến cuộc sống hằng ngày.',glIntro:'Theo dõi thanh toán, đặt lịch nhận nhà, cập nhật tin tức và kết nối với chủ đầu tư — trên một ứng dụng của Gamuda Land.',glReleased:'Đã phát hành trên iOS & Android',glMediaCredit:'Ảnh giới thiệu chính thức từ Google Play · Gamuda Land',gdosLine:'Vận hành bất động sản & bàn giao',aienLine:'Nền tảng học tập có AI hỗ trợ',
...
function renderProject(id: string) {
  ...
  dialogContent.innerHTML = `<h2 id="dialog-title">${escapeHtml(project.title)}</h2><p class="dialog-lede">${escapeHtml(t(project.lede))}</p>${id === 'gl' ? glShowcase() : ''}${[project.context,project.contribution].map((section,index)=>`<section class="dialog-section"><h3>${labels[index]}</h3><p>${escapeHtml(t(section))}</p></section>`).join('')}<details class="technical-details"><summary>${labels[2]}</summary><p>${escapeHtml(t(project.architecture))}</p><ol class="architecture" aria-label="${language === 'en' ? 'Architecture overview' : 'Kiến trúc tổng quan'}">${project.flow.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ol><p class="dialog-tech">${escapeHtml(project.stack)}</p></details><p class="dialog-limit${id === 'gl' ? ' team-credit' : ''}">${escapeHtml(t(project.limit))}</p>${project.source ? `<div class="dialog-footer"><a class="text-link" href="${project.source}" target="_blank" rel="noopener noreferrer">${labels[3]} <svg class="arrow" aria-hidden="true"><use href="#icon-arrow" /></svg></a></div>` : ''}`;
}
