...
const vi: Record<string, string> = {
  ...
  caseStudy:'Xem chi tiết',roberpLine:'Quản lý doanh nghiệp cho thị trường Pháp — từ nhân sự, tài liệu đến vận hành.',roberpKind:'Đóng góp cả frontend & backend',roberpArtCaption:'Minh họa: mỗi công ty có không gian làm việc riêng.',glLine:'Đồng hành từ ngày mua nhà đến cuộc sống hằng ngày.',glIntro:'Theo dõi thanh toán, đặt lịch nhận nhà, cập nhật tin tức và kết nối với chủ đầu tư — trên một ứng dụng của Gamuda Land.',glReleased:'Đã phát hành trên iOS & Android',glMediaCredit:'Ảnh giới thiệu chính thức từ Google Play · Gamuda Land',gdosLine:'Vận hành bất động sản & bàn giao',aienLine:'Nền tảng học tập có AI hỗ trợ',
...
const projects: Record<string, Project> = {
  ...
  roberp:{title:'ROBERP',type:['Team contribution · Frontend & backend · France','Đóng góp trong đội ngũ · Frontend & backend · Pháp'],
    lede:['Business management for French organizations, with a workspace for each company.','Phần mềm quản lý doanh nghiệp cho thị trường Pháp, với không gian làm việc riêng theo từng công ty.'],
    context:['ROBERP brings HR, planning, documents, purchasing, billing and operational workflows into one platform for French companies and associations. Multiple organizations use the system with their own configuration, structures, users and data scope. It also supports workflows for social and care-sector organizations, such as beneficiary records and quality management.','ROBERP tập hợp quản lý nhân sự, lịch làm việc, tài liệu, mua hàng, hóa đơn và các nghiệp vụ vận hành cho doanh nghiệp, hiệp hội tại Pháp. Nhiều công ty cùng sử dụng nền tảng nhưng có cấu hình, cơ cấu tổ chức, người dùng và phạm vi dữ liệu riêng. Sản phẩm còn có các nghiệp vụ cho tổ chức xã hội và chăm sóc, như hồ sơ người được hỗ trợ và quản lý chất lượng.'],
...
function renderProject(id: string) {
  ...
  const projectArt = id === 'roberp' ? `<figure class="project-art"><img src="/images/roberp-workspaces.svg" width="760" height="440" alt="${language === 'en' ? 'Concept: separate company workspaces connected to ROBERP' : 'Minh họa các không gian công ty riêng kết nối với ROBERP'}" /><figcaption>${language === 'en' ? 'Concept illustration · fictional companies A, B and C' : 'Hình minh họa · các công ty A, B, C là ví dụ'}</figcaption></figure>` : '';
  dialogContent.innerHTML = `<h2 id="dialog-title">${escapeHtml(project.title)}</h2><p class="dialog-lede">${escapeHtml(t(project.lede))}</p>${id === 'gl' ? glShowcase() : projectArt}${[project.context,project.contribution].map((section,index)=>`<section class="dialog-section"><h3>${labels[index]}</h3><p>${escapeHtml(t(section))}</p></section>`).join('')}<details class="technical-details"><summary>${labels[2]}</summary><p>${escapeHtml(t(project.architecture))}</p><ol class="architecture" aria-label="${language === 'en' ? 'Architecture overview' : 'Kiến trúc tổng quan'}">${project.flow.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ol><p class="dialog-tech">${escapeHtml(project.stack)}</p></details><p class="dialog-limit${id === 'gl' || id === 'roberp' ? ' team-credit' : ''}">${escapeHtml(t(project.limit))}</p>${project.source ? `<div class="dialog-footer"><a class="text-link" href="${project.source}" target="_blank" rel="noopener noreferrer">${labels[3]} <svg class="arrow" aria-hidden="true"><use href="#icon-arrow" /></svg></a></div>` : ''}`;
}
function updateLanguage(next: Language) {
  ...
  document.querySelector('.roberp-art img')!.setAttribute('alt',next === 'en' ? 'Concept: three company workspaces connected to the ROBERP platform' : 'Minh họa ba không gian công ty kết nối với nền tảng ROBERP');
